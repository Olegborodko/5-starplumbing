...
class Service_Sidebar_Menu extends \WP_Widget {
	...
	public function __construct() {
		 $widget_ops = array(
			 'classname'                   => 'sidebar-widget categories',
			 'description'                 => __( 'Add a Plumbio Services custom menu to your sidebar.' ),
			 'customize_selective_refresh' => true,
		 );
		 parent::__construct( 'Service_Sidebar_Menu', __( 'Plumbio Service Menu' ), $widget_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $title ) {
			...
			printf( $args['before_title'] . get_the_title() . $args['after_title'] );
		}

		$menu_no = wp_get_nav_menu_items( $nav_menu );
		if ( ! empty( $menu_no ) ) {

			echo '<div class="tt-aside02__content"><ul class="tt-aside-menu">';
			...
			foreach ( $primaryNav as $navItem ) {
				...
				if ( $object_post_id == $post_object_id ) {
					echo '<li class="active"><a  href="' . $navItem->url . '" title="' . $navItem->title . '">' . $navItem->title . '</a></li>';
				} else {
